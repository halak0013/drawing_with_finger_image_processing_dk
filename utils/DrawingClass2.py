...
from utils.dk_connection import ImageReceiver, SocketServer
...
class HandDrawingApp:
    """
    ## A class to represent a hand drawing application using OpenCV and MediaPipe.
    Attributes
    - target_width : int
        The target width of the application window.
    - target_height : int
        The target height of the application window.
    - detection_confidence : float
        The confidence threshold for hand detection.
    - max_hands : int
        The maximum number of hands to detect.
    - source : str
        The source of the input, either "camera" or "sock".
    """
    ...
    def __init__(
        ...
        header_path = os.path.join(current_dir, "../Header/header.png")
        self.header = cv2.imread(header_path, cv2.IMREAD_UNCHANGED)
    ...
    def clear_all(self, hands, img):
        """
        It clears the canvas if all fingers are up.\n
        It waits for 10 frames to clear the canvas!
        """
        finger_up = self.detector.fingersUp(hands[0])
        if (
            finger_up[0]
            ...
            cv2.putText(
                img,
                str(10 - self.clear_time),
                (self.target_width // 2, 150),
    ...
    def draw(self, hands, img: cv2.typing.MatLike):
        """
        It draws on the canvas using the one finger.
        """
        if hands:
            x, y, z = self.finger_position(hands)
            cv2.circle(
                img, (x, y), 10, self.brushes[self.brush_index]["color"], cv2.FILLED
            )
            ...
            if self.brush_index == 3:
                cv2.line(
                    ...
                    (self.xp, self.yp),
                    (x, y),
                    self.brushes[self.brush_index]["color"],
                    30,
                )
            else:
                cv2.line(
    ...
    def masking(self, img):
        """Generate a mask where all non-black pixels are white and black pixels are black\n
    ...
    def selecting(self, finger_pos):
        """
        It selects the brush using the finger position.
        with percetage of the screen width
        """
    ...
    def run(self):
        while True:
            self.module_image = success, img = self.get_img()
            if not success:
                print(self.source)
                continue
            img = cv2.flip(img, 1)
            ...
            # Overlay header
            img = cvzone.overlayPNG(img, self.brushes[self.brush_index]["img"], [0, 0])
